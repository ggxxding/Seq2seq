...
def main():
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for var_name, _ in tf.compat.v1.train.list_variables(args.checkpoint_path):

            # 得到上述参数的值
            var = tf.compat.v1.train.load_variable(args.checkpoint_path, var_name)
        ...
        # 构造一个保存器
        saver = tf.compat.v1.train.Saver(var_list=new_var_list)
        # 初始化一下参数（这一步必做）
